...
namespace WoG.Characters.Services.Api.Controllers
{
    ...
    [ApiController]
    public class BaseItemApiController : ControllerBase
    {
        ...
        public BaseItemApiController(ApplicationDbContext dbContext, IConnectionMultiplexer connectionMultiplexer,
        ...
        public async Task<ObjectResult> Get(Guid id)
        {
            try
            {
                var response = await GetBaseItemFromCacheOrDb(id);
                return this.StatusCode(StatusCodes.Status200OK, response);
            }
        ...
        private async Task<BaseItemDto> GetBaseItemFromCacheOrDb(Guid id)
        {
            var cachedData = await redisDatabase.StringGetAsync($"{nameof(BaseItemDto)}:{id}");

            if (cachedData.HasValue)
            {
                return JsonConvert.DeserializeObject<BaseItemDto>(cachedData!)!;
            }
            else
            {
                ...
                var mapped = this.mapper.Map<BaseItemDto>(data);

                await redisDatabase.StringSetAsync($"{nameof(BaseItemDto)}:{id}",
